...
def segment_and_convert(input_file, output_file, start_time, end_time, sample_rate=24000, channels=1, sample_width=16):
    ffmpeg_cmd = f'ffmpeg -i "{input_file}" -ss {start_time} -to {end_time} ' \
                 f'-ar {sample_rate} -ac {channels} -sample_fmt s16 "{output_file}"'
    subprocess.call(ffmpeg_cmd, shell=True)
    print(f"切分并转换完成: {output_file}")

def process_mp4(args, start_idx, chunk_num):
    total_duration=0.0
    ...
    for i, line in enumerate(f_scp):
        ...
        transcription = f"{transcript_path}/{utt}.tsv"
        ...
        with open(transcription) as tsv_file:
            ...
            data = data[1:]  # 跳过第一行标题
            if len(data) == 0:                                
                logger.warning(f"wav_path:{wav}, transcription:{utt}.tsv is empty, continue.")
                continue

        # 打开音频文件
        video = VideoFileClip(wav)
        audio = video.audio
        ...
        for i, line in enumerate(data):
            ...
            if len(paragraph) !=3: # 没有转写内容
                logger.warning(f"transcription:{utt}.tsv TSV format is not 'start\tend\ttext', check it.")
                continue
        ...
        if not len(audio_segments) == len(paragraphs):
            logger.error(f"{utt}.tsv: audio segment and text length not equal.")
            continue
...
def process_scp(args, start_idx, chunk_num):
    ...
    for i, line in enumerate(tqdm(f_scp)):
        ...
        with open(transcription) as tsv_file:
            ...
            if len(data) == 0:
                logger.warning(
                    f"wav_path:{wav}, transcription:{utt}.tsv is empty, continue.")
                continue
        ...
        try:
            ...
            for i, line in enumerate(data):
                ...
                if len(paragraph) != 3:  # 没有转写内容
                    logger.warning(
                        f"transcription:{utt}.tsv TSV format is not 'start\tend\ttext', check it.")
                    continue
                segment_start_time = float(paragraph[0])
                if start_time == None:  # 设置第一段时间起始点
                    start_time = segment_start_time
