...
CtcPrefixDecoder::CtcPrefixDecoder(
  Vocab* vocab, float glob_beam, float lat_beam) 
  :opts_(), vocab_(vocab)
{
  opts_.first_beam_size = static_cast<int>(glob_beam);
  opts_.second_beam_size = static_cast<int>(lat_beam);
  Reset();
}
...
void CtcPrefixDecoder::StartUtterance() {
  Reset();
}

void CtcPrefixDecoder::EndUtterance() {
...
void CtcPrefixDecoder::UpdateOutputs(
...
// it.
std::string CtcPrefixDecoder::CtcSearch(std::vector<std::vector<float>> logp) {
  ...
  std::string text = CtcFinalizeDecode();
  return text;
}
...
void CtcPrefixDecoder::UpdateResult() {
  result_.clear();

  for (size_t i = 0; i < hypotheses_.size(); i++) {
    const std::vector<int>& hypothesis = hypotheses_[i];

    DecodeResult path;
    path.score = likelihood_[i];
    int offset = 0;
    for (size_t j = 0; j < hypothesis.size(); j++) {
      std::string word = vocab_->Id2String(hypothesis[j]);
      path.sentence += (word);
    }
    ...
    if (vocab_ != nullptr )  // modified: we need time stamp in decoding.
    {
      const std::vector<int>& input = hypotheses_[i];
      const std::vector<int>& time_stamp = times_[i];
      CHECK_EQ(input.size(), time_stamp.size());
      for (size_t j = 0; j < input.size(); j++) {
        std::string word = vocab_->Id2String(input[j]);
        int start = 0;
        int end = time_stamp[j];
